...
import com.clarionmedia.infinitum.aop.impl.XmlAspectWrapper.XmlAdviceWrapper;
...
public class XmlAspectWeaver implements AspectWeaver {
	...
	private List<XmlAspectWrapper> mAspects;
	...
	public XmlAspectWeaver(InfinitumContext context,
		...
		mAspects = new ArrayList<XmlAspectWrapper>();
		for (XmlAspect aspect : aspects)
			mAspects.add(new XmlAspectWrapper(aspect));
	}
	...
	/**
	 * Returns a {@link List} of {@link XmlAspect} instances for this
	 * {@code XmlAspectWeaver}.
	 * 
	 * @return {@code List} of {@code XmlAspects}
	 */
	public List<XmlAspect> getAspects() {
		List<XmlAspect> aspects = new ArrayList<XmlAspect>();
		for (XmlAspectWrapper aspect : mAspects)
			aspects.add(aspect.unwrap());
		return aspects;
	}

	/**
	 * Sets the {@link XmlAspect} instances for this
	 * {@code XmlAspectWeaver}.
	 * 
	 * @param aspects
	 *            the {@code XmlAspects} to set
	 */
	public void setAspects(List<XmlAspect> aspects) {
		mAspects = new ArrayList<XmlAspectWrapper>();
		for (XmlAspect aspect : aspects)
			mAspects.add(new XmlAspectWrapper(aspect));
	}
	...
	private Collection<Pointcut> getPointcuts(List<XmlAspectWrapper> aspects) {
		Map<String, Pointcut> pointcutMap = new HashMap<String, Pointcut>();
		// Process all aspects
		for (XmlAspectWrapper aspect : aspects) {
	...
	private void processAspect(XmlAspectWrapper aspect,
			Map<String, Pointcut> pointcutMap) {
		for (XmlAdviceWrapper advice : aspect.getAdvice()) {
	...
	private void processBeanJoinPoints(Object advisor,
			XmlAdviceWrapper advice, Method adviceMethod,
			Map<String, Pointcut> pointcutMap) {
	...
	private void processBeanMethodJoinPoint(String bean, Object beanObject,
			Object advisor, XmlAdviceWrapper advice, JoinPoint joinPoint,
			Map<String, Pointcut> pointcutMap) {
		if (!bean.endsWith(")"))
